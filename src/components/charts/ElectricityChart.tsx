...
const ElectricityChart: React.FC<ElectricityChartProps> = ({
  ...
  if (!context) {
    throw new Error(
      "ElectricityChart must be used within a NavigationContextProvider"
    );
  }
  const { isExpertMode, electricityAggregation, setElectricityAggregation } =
    context;

  const [plotDataFromWorker, setPlotDataFromWorker] =
    useState<AggregatedPlotData | null>(null);
  const [isProcessing, setIsProcessing] = useState<boolean>(true);
  const [lastError, setLastError] = useState<string | null>(null);

  const [selectedCountries, setSelectedCountries] = useState<string[]>(
    initialMetadata.availableCountries
  );
  ...
  const effectiveAggregationLevel = isExpertMode
  ...
  useEffect(() => {
    ...
    const handleWorkerMessage = (event: MessageEvent<FromWorkerMessage>) => {
      ...
      switch (event.data.type) {
        ...
        case "AGGREGATION_ERROR":
          console.error(
            "Chart: Worker reported aggregation error:",
            event.data.error
          );
        ...
        case "PARSE_ERROR":
          console.error(
            "Chart: Worker reported parse error:",
            event.data.error
          );
          setLastError(event.data.error);
          setPlotDataFromWorker(null);
    ...
    const initialMessage: ToWorkerMessage = {
      type: "AGGREGATE",
  ...
  const { chartDataForPlot, chartLayoutUpdates } = useMemo(() => {
    if (!plotDataFromWorker)
      return { chartDataForPlot: [], chartLayoutUpdates: {} };
        ...
        finalTraces = plotDataFromWorker
          ...
          .map((trace, index) => {
            ...
            const hoverText = traceY.map((origY, i) => {
              ...
              const percentage =
                total > 0
                  ? (((Number(origY) || 0) / total) * 100).toFixed(1) + "%"
                  : "0.0%";
              return `Value: ${(Number(origY) || 0).toFixed(
                0
              )} MWh (${percentage})`;
  ...
  if (lastError) {
    return (
      <p style={{ color: "red" }}>
        Error processing electricity data: {lastError}
      </p>
    );
  }
  ...
  const noDataMessage =
    ...
    (!plotDataFromWorker || plotDataFromWorker.length === 0) ? (
      <p>
        No Electricity Generation data available for the selected countries or
        filters.
      </p>
    ) : null;
  ...
  if (
    chartTypes[currentChartTypeIndex].key === "stacked-area" &&
    chartLayoutUpdates.yaxis?.range?.[1] === 100
  ) {
    ...
    >
      {isExpertMode && (
        <Box sx={{ marginBottom: "20px", textAlign: "center" }}>
          <ButtonGroup
            variant="outlined"
            aria-label="Electricity Aggregation Level"
          >
            <Button
          ...
          {chartTypes.map((type, index) => (
            <Box
              ...
              sx={{
                ...
                borderRadius: "50%",
                backgroundColor:
                  currentChartTypeIndex === index ? "primary.main" : "grey.400",
                cursor: "pointer",
              ...
              tabIndex={0}
              onKeyPress={(e) =>
                e.key === "Enter" && handleSelectChartType(index)
              }
            />
          ))}
        </Box>
