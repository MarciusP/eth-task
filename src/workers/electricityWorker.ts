...
async function handleInitialization(parquetData: ArrayBuffer, wasmPath: string): Promise<ParseSuccessMessage | ParseErrorMessage> {
    try {
        ...
        const availableCountries = Array.from(new Set(rawData.map(d => d.country_id))).sort();
        const dates = rawData.map(d => new Date(Number.isNaN(Number(d.datetime)) ? d.datetime : Number(d.datetime))).filter(d => !isNaN(d.getTime()));
        const minDate = dates.length > 0 ? dates.reduce((min, p) => p < min ? p : min, dates[0]) : null;
        const maxDate = dates.length > 0 ? dates.reduce((max, p) => p > max ? p : max, dates[0]) : null;
        ...
        isInitialized = false;
        rawData = [];
        return { type: "PARSE_ERROR", error: error.message || "Unknown parsing error" };
    }
}
...
function performAggregation(request: AggregationRequest): AggregatedPlotData {
    ...
    const filteredData = rawData.filter(d => selectedCountries.includes(d.country_id));
    ...
    filteredData.forEach((d) => {
        ...
        if (!aggregatedData[periodKey][d.country_id]) aggregatedData[periodKey][d.country_id] = {};
    ...
    for (const country of countriesToProcess) {
      for (const tech of technologies) {
        const yValues = periods.map((period) => 
          aggregatedData[period]?.[country]?.[tech] || 0
        );
...
self.onmessage = async (event: MessageEvent<ToWorkerMessage>) => {
    ...
    try {
        switch (message.type) { // Use message.type inside switch
            case "INIT_AND_PARSE":
                if (isInitialized) {
                    response = { type: "PARSE_ERROR", error: "Worker already initialized" };
                } else {
                   response = await handleInitialization(message.parquetData, message.wasmPath);
                }
                break;

            case "AGGREGATE":
                if (!isInitialized || !rawData) {
                    console.error("Worker: Cannot aggregate before successful initialization.");
                    response = { type: "AGGREGATION_ERROR", error: "Worker not initialized or no data." };
                } else {
        ...
        console.error("Worker: Unhandled error processing message:", messageType, error);
        // Use stored messageType here
        response = { 
            type: messageType === 'AGGREGATE' ? "AGGREGATION_ERROR" : "PARSE_ERROR", 
            error: `Unhandled worker error during ${messageType}: ${error.message || 'Unknown error'}` 
        };
    }
